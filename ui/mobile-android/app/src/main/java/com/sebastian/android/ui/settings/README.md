...
├── AgentBindingsPage.kt  # Agent LLM 绑定页（每个 Sub-Agent 独立选 Provider）
...
Agent LLM 绑定页，由 `AgentBindingsViewModel` 驱动。展示所有 Sub-Agent 列表，每个 Agent 可独立绑定指定 Provider 或回退使用全局默认。点击 Agent 弹出 `ModalBottomSheet` 选择 Provider。绑定成功/失败均通过 `ToastCenter` 提示用户。
...
| 改 Agent LLM 绑定逻辑 | `AgentBindingsPage.kt` + `viewmodel/AgentBindingsViewModel.kt` |
