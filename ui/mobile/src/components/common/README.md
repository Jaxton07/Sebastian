...
├── ConfirmDialog.tsx           # 通用确认弹窗（iOS 风格，支持 destructive 模式）
├── BackButton.tsx              # 通用返回按钮（左箭头 + 返回，默认调用 router.back()）
...
| 修改通用确认弹窗样式或交互 | [ConfirmDialog.tsx](ConfirmDialog.tsx) |
| 修改全局返回按钮样式（箭头/文案/色彩） | [BackButton.tsx](BackButton.tsx) |
