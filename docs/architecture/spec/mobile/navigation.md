...
| 场景 | 屏幕宽度（dp）| 模式 | 说明 |
|------|-------------|------|------|
| 手机竖屏 | 360–430 | 覆盖（Overlay）| 侧栏滑出叠加在主内容上，主内容露出约 20% |
| 手机横屏 | 700–900 | 覆盖 | 宽度仍不足 1000dp |
| 平板竖屏 | 800–960 | 覆盖 | 宽度仍不足 1000dp |
| 平板横屏 | 1200–1600 | 常驻（Persistent）| 左右面板固定展开，主内容与侧栏并排 |
| Z Fold 展开横屏 | ~1200 | 常驻 | 铰链处天然分隔左右两栏 |

> 这一规则同时满足：手机始终覆盖式、平板横屏才常驻，不需要监听旋转事件。
...
左右侧栏互斥打开（手机覆盖模式），或同时常驻（平板 Persistent 模式）。
...
| 手势 | 行为 |
|------|------|
| 从左边缘右滑 | 拉出 SessionPanel |
| 从右边缘左滑 | 拉出 TodoPanel |
| 侧栏展开时点击主内容区 | 关闭侧栏，回到主内容 |
| 侧栏展开时侧滑关闭 | 反向滑动关闭 |
