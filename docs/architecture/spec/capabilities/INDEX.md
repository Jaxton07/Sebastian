...
| [skill-package-manager.md](skill-package-manager.md) | Sebastian Skill 包管理器与 progressive disclosure runtime：Skill 是本地 catalog package、CLI list/search/show/read 按需读取，本地 search 支持多语言 multi-token OR 匹配与确定性排序，ClawHub-compatible registry consumer、lockfile、安全解压、PATH shim、builtin `skill_manager` |
