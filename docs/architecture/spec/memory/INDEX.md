...
截至 2026-04-19，Memory 系统已完成 Phase A / B 的主体实现，并完成 Phase C / D 的部分能力：

| Phase | 当前状态 | 说明 |
|-------|----------|------|
| A 协议先行 | 已实现 | `MemoryArtifact`、Slot Registry、Resolver、Retrieval Plan、Assembler、Decision Log 主协议已落地 |
| B Profile + Episode 可用版 | 已实现，审查修复中 | Profile / Episode 存储、写入、检索、注入与基础工具已可用；当前审查修复聚焦 `valid_from`、summary-first episode lane、`input_source` 等一致性问题 |
| C Consolidation 成熟版 | 部分实现 | Session Consolidation、startup catch-up sweep 与 EXPIRE 类生命周期处理已实现；Cross-Session Consolidation、周期性 Maintenance、降权、重复压缩、索引修复仍需单独 spec |
| D Relation / Graph 增强 | 部分实现 | Entity Registry、`relation_candidates` 与 Relation Lane 已实现；exclusive relation 时间边界、正式 relation facts / graph 查询语义仍需单独 spec |
