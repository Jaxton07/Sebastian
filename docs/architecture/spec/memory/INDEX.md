...
| E 常驻记忆快照 | 已实现 | `ResidentMemorySnapshotRefresher` + `resident_dedupe.py`；快照按用户画像高置信记录预渲染为 Markdown，每轮固定注入（base → runtime → resident → dynamic → todos）；动态检索去重，避免重复注入；详见 [resident-snapshot.md](resident-snapshot.md) |
