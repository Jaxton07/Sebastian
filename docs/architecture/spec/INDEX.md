...
| [artifact-model.md](memory/artifact-model.md) | MemoryArtifact（记忆产物）、Slot Registry（语义槽位注册表）、生命周期、冲突决策模型 |
| [storage.md](memory/storage.md) | Profile Store（画像存储）、Episode Store（经历存储）、Entity Registry（实体注册表）、Relation Layer（关系层）、Decision Log（决策日志） |
| [write-pipeline.md](memory/write-pipeline.md) | Capture（捕获）→ Extract（提取）→ Normalize（规范化）→ Resolve（冲突解析）→ Persist（持久化）统一写入链路 |
| [retrieval.md](memory/retrieval.md) | Intent（意图）→ Retrieval Planner（检索规划器）→ Assembler（上下文装配器），四条 retrieval lane（检索通道） |
...
| [implementation.md](memory/implementation.md) | 首版 DB + LLM 技术边界、memory_extractor / memory_consolidator 模型绑定、结构化 schema |
