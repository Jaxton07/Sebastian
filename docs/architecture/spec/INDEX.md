...
| [system-prompt.md](core/system-prompt.md) | Sebastian 人设提示词、BASE_BUTLER_RULES + soul 文件热切换、BaseAgent 结构化 prompt 构造体系（persona/tools/skill management/agents/knowledge）、每轮 Runtime Context 当前时间注入、Bash-only Skill-first / browser-last discovery bootstrap、per-agent 工具可见性 |
...
| [skill-package-manager.md](capabilities/skill-package-manager.md) | Sebastian Skill 包管理器与 progressive disclosure runtime：Skill 是本地 catalog package、CLI list/search/show/read 按需读取，本地 search 支持多语言 multi-token OR 匹配与确定性排序，ClawHub-compatible registry consumer、lockfile、安全解压、PATH shim、builtin `skill_manager` |
