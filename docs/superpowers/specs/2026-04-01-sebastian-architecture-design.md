...
Task 是 Sebastian 的核心实体，不是 Session 的子集。

| 概念 | Session（旧） | Task（新） |
|------|--------------|------------|
| 生命周期 | 用户发起，对话结束即止 | 持久存在，跨 turn/跨会话 |
| 执行方式 | 同步等待用户 | 后台异步，可 pause/resume |
| 状态持久化 | 内存 | SQLite 检查点 |
| 跨设备 | 不支持 | Mobile 可监看/控制 |
...
│  Notification Broker · Context Switcher              │
├─────────────────────────────────────────────────────┤
│              Event Bus（解耦两平面）                  │
├─────────────────────────────────────────────────────┤
│  任务平面（异步 · 持久 · 后台运行）                   │
│  Task Planner · Task Executor · Task Store           │
│  Concurrency Governor · Checkpoint Manager           │
└─────────────────────────────────────────────────────┘
```

两平面通过 Event Bus 解耦：打断是事件，恢复是事件，任何一侧不直接调用另一侧内部方法。
...
| 持久化 | SQLite + SQLAlchemy（async） | 自托管零依赖，个人规模足够 |
...
class Task(BaseModel):
    ...
    checkpoints: list[Checkpoint]    # 执行检查点列表
...
| 功能模块 | 说明 |
|----------|------|
| 对话界面 | 主屏幕，与 Sebastian 实时对话，消息流式显示 |
| 任务看板 | 后台任务列表，显示状态/进度/耗时 |
| 任务详情 | 单任务的执行步骤、事件流、中间产出 |
...
```
# 对话
POST   /api/v1/turns                    # 发送消息或高层指令
GET    /api/v1/turns/{session_id}       # 获取对话历史

# 任务管理
GET    /api/v1/tasks                    # 任务列表（支持分页、状态过滤）
...
# 事件流（SSE）
GET    /api/v1/stream                   # 全局实时事件流（含所有任务、对话）
GET    /api/v1/tasks/{id}/stream        # 单任务事件流
...
| OpenJax | Sebastian |
|---------|-----------|
| Session 为核心 | Task 为核心一等公民 |
| 对话驱动，同步执行 | 双平面，异步持久执行 |
| 手动注册工具 | 目录扫描自动注册（tools/mcps/skills 三层） |
| 无 Sub-Agent | A2A 多 Agent 编排 |
