...
5. **打断执行**：取到 `state.agent_instances[agent_type]`，调 `agent.cancel_session(session_id)`（sebastian/core/base_agent.py:581）。这会 cancel `_active_streams` 里该 session 的 asyncio.Task
...
- **Sebastian**（`sebastian/orchestrator/` 对应配置）：allowed_tools 中 `reply_to_agent` → `resume_agent`；新增 `stop_agent`
- **所有组长 agent**（`sebastian/agents/*/manifest.toml`）：如果当前有 `reply_to_agent` 则改名；如果组长应该能停自己的组员，则新增 `stop_agent`

### 6.2 组员（depth=3）

组员**不注册** stop_agent 或 resume_agent——组员不应管理 sibling 或自己。这由 allowed_tools 白名单约束即可，不需要代码层硬编码 depth 检查。
...
| `sebastian/agents/*/manifest.toml` | 组长 agent allowed_tools 同步 |
