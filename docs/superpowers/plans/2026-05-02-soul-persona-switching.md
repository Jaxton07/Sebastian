...
**Architecture:** `SoulLoader` 负责 `~/.sebastian/data/souls/` 目录管理；`app_settings` KV 表存储 `active_soul`；`switch_soul` 工具通过 `gateway.state` 单例引用更新 `sebastian.persona` 和 `system_prompt`；gateway lifespan 启动时恢复上次切换的 soul。
...
- [ ] **Step 1: 在 `sebas.py` 添加 CORTANA_PERSONA**

在 `SEBASTIAN_PERSONA` 常量之后（`sebastian/orchestrator/sebas.py` 约第 86 行）插入：
