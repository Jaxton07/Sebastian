...
P0 阶段新增了两个子包，建立统一的外部访问边界。P1 阶段将内部实现文件重组到 `stores/`、`writing/`、`retrieval/`、`consolidation/`、`resident/` 五个子包。
...
`retrieve_memory_section()`（`retrieval/retrieval.py`）和 `process_candidates()`（`writing/pipeline.py`）是 P1 内部实现，由 `services/` 层封装调用，不直接对外暴露。
...
│   ├── retrieval_lexicon.py # 静态词库：PROFILE_LANE_WORDS / CONTEXT_LANE_WORDS / EPISODE_LANE_WORDS 等
│   ├── depth_guard.py       # 检索深度保护：depth >= 2 的 agent 不进入检索流程
...
| [writing/slots.py](writing/slots.py) | 提供 `SlotRegistry`、内置 `SlotDefinition`（含 `user.preference.addressing`）和 `DEFAULT_SLOT_REGISTRY`；当前校验 `fact` / `preference` 必须绑定已注册 slot |
| [retrieval/segmentation.py](retrieval/segmentation.py) | 提供基于 `jieba.cut_for_search()` 的 FTS5 中文分词辅助：`segment_for_fts()`、`terms_for_query()`、`add_entity_terms()` |
| [writing/decision_log.py](writing/decision_log.py) | 提供 `MemoryDecisionLogger.append()`，把 `ResolveDecision` 写入 `MemoryDecisionLogRecord` |
...
### Provider Bindings（`consolidation/provider_bindings.py`）
...
### MemoryExtractor（`consolidation/extraction.py`）
...
### MemoryConsolidator（`consolidation/consolidation.py`）
...
### process_candidates（`writing/pipeline.py`）
...
### SessionConsolidationWorker（`consolidation/consolidation.py`）
...
### MemoryConsolidationScheduler（`consolidation/consolidation.py`）
...
### Dynamic Slot System（`stores/slot_definition_store.py` + `writing/slot_proposals.py` + `consolidation/prompts.py` + `writing/feedback.py`）
...
| 功能 | 状态 | 说明 |
|------|------|------|
| current truth 过滤 | 已实现 | `stores/profile_store.py` 三个查询方法加 `valid_from <= now` 过滤；`retrieval/retrieval.py` Assembler `_keep()` 加 `status/subject_id/valid_from` 二次过滤 |
| ExtractorInput.task 契约字段 | 已实现 | `ExtractorInput.task: Literal["extract_memory_artifacts"]`，与 spec §6 对齐 |
| Episode Lane summary-first | 已实现 | `stores/episode_store.py` 新增 `search_summaries_by_query`、`search_episodes_only`；检索 pipeline 先查 summary，不足时再补 episode detail |
...
| SQLite FTS5 中文预分词、查询 term 生成、实体词注入 | [retrieval/segmentation.py](retrieval/segmentation.py) |
| 记忆冲突/写入决策审计日志 | [writing/decision_log.py](writing/decision_log.py) |
| Profile 画像的写入、查询、supersede | [stores/profile_store.py](stores/profile_store.py) |
| 经历事件的写入与 FTS 检索 | [stores/episode_store.py](stores/episode_store.py) |
| 每轮记忆检索 pipeline（planner → fetch → assemble） | [retrieval/retrieval.py](retrieval/retrieval.py) |
...
| 动态 slot 的 DB 存储（memory_slots 表 CRUD） | [stores/slot_definition_store.py](stores/slot_definition_store.py) |
| 动态 slot 校验、注册、并发 race 保护 | [writing/slot_proposals.py](writing/slot_proposals.py) |
| memory_save 结构化结果与自然语言摘要渲染 | [writing/feedback.py](writing/feedback.py) |
| 常驻记忆快照读写、脏标记、重建触发 | [resident/resident_snapshot.py](resident/resident_snapshot.py) |
| 常驻记忆去重纯函数（canonical_bullet、slot_value_dedupe_key 等） | [resident/resident_dedupe.py](resident/resident_dedupe.py) |
